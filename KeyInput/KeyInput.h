...
public:
	...
	static char keys[KEY_MAX_NUM];
	static char preKeys[KEY_MAX_NUM];

public:
	// ゲッター
	static char getKeys(int num);
	static char getPreKeys(int num);

	// すべてのボタンの状態をチェック
	static void Input();

	// 押された瞬間
	static bool Pushed(int num);

	// 長押し
	static bool LongPush(int num);

	// 離した瞬間
	static bool Released(int num);
};